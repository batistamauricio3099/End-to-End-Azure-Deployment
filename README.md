...
Automated Logic: Custom scripting (Bash) to dynamically retrieve and inject credentials without human intervention.
...
⚙️ How it Works
Validation: The pipeline runs a Bicep linting and "What-If" analysis to ensure the code is valid.

Infrastructure Deployment: Azure resources are provisioned following the least-privilege principle.

Secret Injection: Scripts automatically fetch connection strings from the new resources and securely store them as Secrets in Azure Key Vault.

Application Release: The build artifact is deployed to the Linux App Service.

👨‍💻 Author
Mauricio Batista Systems Engineer | DevSecOps specialist [LinkedIn Profile](https://www.linkedin.com/in/mauriciobatista30/)
